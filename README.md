...
**해결방법:**
1. **ToggleTerm 설정 확인**: 플러그인이 올바르게 설치되어 있는지 확인
2. **실시간 출력**: 명령어에 `-verbose` 옵션이 추가되어 있어 더 상세한 출력 제공
3. **터미널 크기 조정**: `:KDSBuild` 실행 후 터미널 창 크기를 조정해보세요
4. **스크롤백 확인**: 터미널에서 위로 스크롤하여 출력 내용을 확인해보세요
5. **대안**: `:terminal` 명령으로 직접 터미널을 열고 KDS 명령어를 수동으로 실행해보세요
